...
def evaluate(checkpoint):
    with tf.Graph().as_default() as g:
        ...
        with tf.Session(config=tf.ConfigProto(
            ...
            np.savetxt('./log_eval_for_predicted_value_dictribution', total_predicted_value[range(0,100)], fmt='%.4f')
            ...
            np.savetxt('./log_eval_for_predicted_value', total_predicted_value[range(0,100)], fmt='%.4f')
            
            detail_filename = os.path.join(graphcnn_option.DATA_PATH,'remap')
