    ...
    with tf.Graph().as_default() as g, tf.device('/cpu:0'):
        ...
        with tf.Session(config=tf.ConfigProto(
            ...
            while step < num_iter:
                ...
                samples_X_eigenvector = sess.run(
                    conv_vectors, feed_dict={data: test_data})
                total_eval_X_eigenvector_list.append(samples_X_eigenvector)
                step += 1
            total_eval_X_eigenvector = np.concatenate(total_eval_X_eigenvector_list, axis=0)

            assert total_sample_count == total_eval_X_eigenvector.shape[0], 'sample_count error! %d != %d' % (
            total_sample_count, total_eval_X_eigenvector.shape[0])

            total_predicted_value = evaluate_SVM(checkpoint, total_eval_X_eigenvector)
...
def evaluate_SVM(checkpoint, total_eval_X_eigenvector):
    trainDataSet = graphcnn_input.generate_train_data(graphcnn_option.TRAIN_DATA_DIR,
                                                      shuffled=False,
                                                      ont_hot=True,
                                                      index_mode=True)
    ...
    with tf.Graph().as_default() as g, tf.device('/cpu:0'):
        # Get images and labels
        data = tf.placeholder(tf.float32, [graphcnn_input.TRAIN_BATCH_SIZE, graphcnn_input.HEIGHT, graphcnn_input.WIDTH,
                                           graphcnn_input.NUM_CHANNELS])
        labels = tf.placeholder(tf.int32, [graphcnn_input.TRAIN_BATCH_SIZE, graphcnn_input.NUM_CLASSES])
        ...
        with tf.Session(config=tf.ConfigProto(
            ...
            num_iter = int(math.floor(graphcnn_input.NUM_EXAMPLES_PER_EPOCH_FOR_TRAIN / graphcnn_input.TRAIN_BATCH_SIZE))
            total_sample_count = num_iter * graphcnn_input.TRAIN_BATCH_SIZE
            step = 0

            total_samples_X_eigenvector_list = []  ##
            total_true_value_list = []
            while step < num_iter:
                test_data, test_label = trainDataSet.next_batch(graphcnn_input.TRAIN_BATCH_SIZE)
            ...
            total_eval_predicted_value_list = []
            ...
            for bin_class in range(0,graphcnn_input.NUM_CLASSES):
                ...
                result = clf.predict(total_eval_X_eigenvector)
                result = np.reshape(result,[-1,1])
                total_eval_predicted_value_list.append(result)
            ...
            total_eval_predicted_value = np.concatenate(total_eval_predicted_value_list, axis=1)
            return total_eval_predicted_value
...
def main(argv=None):  # pylint: disable=unused-argument
    ...
    str = input('please reset the train batch size:(y/n)')
    if str == 'y' or str == 'Y':
        pass
    elif str == 'n' or str == 'N':
        print('eval end!')
